...
- (id)initWithFrame:(CGRect)frame {
  if (self = [super initWithFrame:frame]) {
    ...
    [self.rewindButton addTarget:self
                ...
                forControlEvents:UIControlEventTouchUpInside];
    [self.rewindButton setImage:[UIImage imageNamed:@"rewind"]
                       forState:UIControlStateNormal];
    ...
    [self.fastForwardButton addTarget:self
                     ...
                     forControlEvents:UIControlEventTouchUpInside];
    [self.fastForwardButton setImage:[UIImage imageNamed:@"forward"]
                            forState:UIControlStateNormal];
    ...
    self.leftGrabHandleView =
        [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"grab_handle"]];
    self.leftGrabHandleView.alpha = 0.2f;
    [self addSubview:self.leftGrabHandleView];

    self.rightGrabHandleView =
        [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"grab_handle"]];
...
- (void)layoutSubviews {
  ...
  self.playPauseButton.frame =
      CGRectMake(CGRectGetMidX(self.bounds) - (kButtonWidth / 2.f), 0.f,
                 kButtonWidth, CGRectGetHeight(self.bounds));

  [self.rewindButton sizeToFit];
  self.rewindButton.frame = CGRectMake(kButtonEdgeInset, 0.f, kButtonWidth,
                                       CGRectGetHeight(self.bounds));

  [self.fastForwardButton sizeToFit];
  self.fastForwardButton.frame =
      CGRectMake(CGRectGetWidth(self.bounds) - kButtonWidth - kButtonEdgeInset,
                 0.f, kButtonWidth, CGRectGetHeight(self.bounds));

  self.elapsedTimeLabel.frame =
      CGRectMake(CGRectGetMinX(self.rewindButton.frame) -
                     CGRectGetWidth(self.elapsedTimeLabel.bounds),
                 0.f, CGRectGetWidth(self.elapsedTimeLabel.bounds),
                 CGRectGetHeight(self.bounds));

  self.remainingTimeLabel.frame =
      CGRectMake(CGRectGetMaxX(self.fastForwardButton.frame), 0.f,
                 CGRectGetWidth(self.remainingTimeLabel.bounds),
                 CGRectGetHeight(self.bounds));

  self.leftGrabHandleView.frame = CGRectMake(
      kGrabHandleInset, CGRectGetMidY(self.bounds) -
                            CGRectGetMidY(self.leftGrabHandleView.bounds),
      CGRectGetWidth(self.leftGrabHandleView.bounds),
      CGRectGetHeight(self.leftGrabHandleView.bounds));

  self.rightGrabHandleView.frame = CGRectMake(
      CGRectGetWidth(self.bounds) -
          CGRectGetWidth(self.rightGrabHandleView.bounds) - kGrabHandleInset,
      CGRectGetMidY(self.bounds) -
          CGRectGetMidY(self.rightGrabHandleView.bounds),
      CGRectGetWidth(self.rightGrabHandleView.bounds),
      CGRectGetHeight(self.rightGrabHandleView.bounds));
}
