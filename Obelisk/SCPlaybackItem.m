...
- (NSString *)stringForRemainingTime {
  NSUInteger hours =
      [self hoursComponentForTimeInterval:self.totalTime - self.elapsedTime];
  NSUInteger minutes =
      [self minutesComponentForTimeInterval:self.totalTime - self.elapsedTime];
  NSUInteger seconds =
      [self secondsComponentForTimeInterval:self.totalTime - self.elapsedTime];

  return [NSString stringWithFormat:@"-%@", [self stringForHours:hours
                                                         minutes:minutes
                                                         seconds:seconds]];
}

- (NSString *)stringForHours:(NSUInteger)hours
                     minutes:(NSUInteger)minutes
                     seconds:(NSUInteger)seconds {
  if (hours > 0) {
    return [NSString stringWithFormat:@"%lu:%lu:%02lu", (unsigned long)hours,
                                      (unsigned long)minutes,
                                      (unsigned long)seconds];
  }
  return [NSString stringWithFormat:@"%lu:%02lu", (unsigned long)minutes,
                                    (unsigned long)seconds];
}
