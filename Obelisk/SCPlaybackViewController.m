...
@interface SCPlaybackViewController ()<UICollisionBehaviorDelegate,
                                       SCControlsViewDelegate>
...
@property(nonatomic, assign, getter=shouldCommitTimelineScrubbing)
    BOOL commitTimelineScrubbing;
...
- (void)viewDidLoad {
  ...
  self.timelineView = [[SCTimelineView alloc]
      initWithFrame:CGRectMake(0.f, kTimelineExpandedHeight * -1,
  ...
  UIView *keylineView =
      [[UIView alloc] initWithFrame:CGRectMake(0.f, kKeylineHeight * -1,
                                               CGRectGetWidth(self.view.bounds),
                                               kKeylineHeight)];
  keylineView.backgroundColor =
      [[UIColor whiteColor] colorWithAlphaComponent:0.25f];
  [self.view addSubview:keylineView];

  self.tapHintBehavior =
      [[SCTapHintBehavior alloc] initWithItem:@[ self.controlsView ]];
  self.tapHintBehavior.collisionBehavior.collisionDelegate = self;
  self.tapHintBehavior.collisionBehavior.translatesReferenceBoundsIntoBoundary =
      NO;

  self.scrubbingBehavior =
      [[SCScrubbingBehavior alloc] initWithItem:self.controlsView
  ...
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(updatePlayback:)
                                               name:@"MediaStatusUpdate"
                                             object:nil];
  if ([GMBChromecastManager sharedManager].mediaControlChannel) {
    NSLog(@"Requesting status");
    [[GMBChromecastManager sharedManager].mediaControlChannel requestStatus];
  }
}

- (void)updatePlayback:(NSNotification *)notification {
  NSLog(@"Updating playback status");
  GCKMediaInformation *info =
      [GMBChromecastManager sharedManager].mediaInformation;
  GCKMediaControlChannel *channel =
      [GMBChromecastManager sharedManager].mediaControlChannel;
  if (info && channel) {
    NSLog(@"Actually updating playback status.");
    _playbackItem.totalTime = info.streamDuration;
    _playbackItem.elapsedTime = channel.approximateStreamPosition;
    [self.timelineView updateForPlaybackItem:_playbackItem];
    [self.controlsView updateForPlaybackItem:_playbackItem];
  }
}
...
- (void)timerDidFire:(id)sender {
  if (self.isTimelineScrubbing == NO &&
      self.playbackItem.elapsedTime < self.playbackItem.totalTime) {
...
- (void)expandTimelineViewAnimated:(BOOL)animated {
  ...
  void (^timelineExpansionBlock)() =
      ^void() { self.timelineView.transform = CGAffineTransformIdentity; };

  void (^completionBlock)(BOOL finished) =
      ^void(BOOL finished) { self.commitTimelineScrubbing = YES; };
...
- (void)collapseTimelineViewAnimated:(BOOL)animated {
  ...
  void (^timelineCollapsingBlock)() = ^void() {
      ...
      CGAffineTransform timelineViewScaleTransform = CGAffineTransformMakeScale(
          1.f, kTimelineCollapsedHeight / kTimelineExpandedHeight);
      CGAffineTransform timelineViewTranslationTransform =
          CGAffineTransformMakeTranslation(
              0.f, kTimelineExpandedHeight / kTimelineCollapsedHeight);

      self.timelineView.transform = CGAffineTransformConcat(
          timelineViewScaleTransform, timelineViewTranslationTransform);
  };

  void (^completionBlock)(BOOL finished) =
      ^void(BOOL finished) { self.commitTimelineScrubbing = NO; };
...
- (void)panGestureRecognized:(UIPanGestureRecognizer *)panGesture {
  ...
  if (panGesture.state == UIGestureRecognizerStateBegan) {
    ...
    CGFloat translatedCenterX =
        self.view.center.x + (translationInView.x - self.touchesBeganPoint.x);

    CGFloat scrubbingProgress =
        (translationInView.x - self.touchesBeganPoint.x) /
        CGRectGetWidth(self.view.bounds);
    NSTimeInterval timeAdjustment =
        self.playbackItem.totalTime * scrubbingProgress;

    self.playbackItem.elapsedTime =
        self.elapsedTimeAtTouchesBegan + timeAdjustment;
    [self.timelineView updateForPlaybackItem:self.playbackItem];
    [self.controlsView updateForPlaybackItem:self.playbackItem];

    self.controlsView.center =
        CGPointMake(translatedCenterX, self.controlsView.center.y);
...
- (void)tapGestureRecognized:(UITapGestureRecognizer *)tapGestureRecognized {
  ...
  if (locationInView.x < CGRectGetMidX(self.view.bounds)) {
    CGPoint leftCollisionPointTop =
        CGPointMake(kCollisionBoundaryOffset * -1, 0.f);
    CGPoint leftCollisionPointBottom = CGPointMake(
        kCollisionBoundaryOffset * -1, CGRectGetHeight(self.view.bounds));

    [self.tapHintBehavior.collisionBehavior
        addBoundaryWithIdentifier:@"leftCollisionPoint"
                        fromPoint:leftCollisionPointTop
                          toPoint:leftCollisionPointBottom];
    [self.tapHintBehavior.gravityBehavior setAngle:(float)M_PI
                                         magnitude:kGravityMagnitude];
    [self.tapHintBehavior.pushBehavior setAngle:0.f magnitude:kPushMagnitude];
  } else {
    CGPoint rightCollisionPointTop = CGPointMake(
        CGRectGetWidth(self.view.bounds) + kCollisionBoundaryOffset, 0.f);
    CGPoint rightCollisionPointBottom =
        CGPointMake(CGRectGetWidth(self.view.bounds) + kCollisionBoundaryOffset,
                    CGRectGetHeight(self.view.bounds));

    [self.tapHintBehavior.collisionBehavior
        addBoundaryWithIdentifier:@"rightCollisionPoint"
                        fromPoint:rightCollisionPointTop
                          toPoint:rightCollisionPointBottom];
    [self.tapHintBehavior.gravityBehavior setAngle:0.f
                                         magnitude:kGravityMagnitude];
    [self.tapHintBehavior.pushBehavior setAngle:(float)M_PI
                                      magnitude:kPushMagnitude];
  }
...
- (void)controlsView:(SCControlsView *)controlsView
    didTapPlayButton:(UIButton *)playButton {
  self.playbackTimer = [NSTimer timerWithTimeInterval:1.
                                               target:self
                                             selector:@selector(timerDidFire:)
                                             userInfo:nil
                                              repeats:YES];
  [[NSRunLoop currentRunLoop] addTimer:self.playbackTimer
                               forMode:NSDefaultRunLoopMode];
}

- (void)controlsView:(SCControlsView *)controlsView
    didTapPauseButton:(UIButton *)playButton {
  [self.playbackTimer invalidate];
}

- (void)controlsView:(SCControlsView *)controlsView
    didTapRewindButton:(UIButton *)playButton {
...
- (void)controlsView:(SCControlsView *)controlsView
    didTapFastForwardButton:(UIButton *)playButton {
  self.playbackItem.elapsedTime =
      fmin(self.playbackItem.totalTime, self.playbackItem.elapsedTime + 30.);
  [self.timelineView updateForPlaybackItem:self.playbackItem];
  [self.controlsView updateForPlaybackItem:self.playbackItem];
}
