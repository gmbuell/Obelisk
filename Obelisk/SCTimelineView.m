...
- (void)updateProgressViewForPlaybackItem:(SCPlaybackItem *)playbackItem {
  if (playbackItem.totalTime > 0.f) {
    CGFloat progress = (float)playbackItem.elapsedTime / (float)playbackItem.totalTime;
    self.progressView.frame =
        CGRectMake(0.f, 0.f, CGRectGetWidth(self.bounds) * progress,
...
- (void)updateElapsedTimeLabelForPlaybackItem:(SCPlaybackItem *)playbackItem {
  ...
  CGSize labelSize = [self.elapsedTimeLabel.text
      sizeWithAttributes:@{NSFontAttributeName : self.elapsedTimeLabel.font}];
  self.elapsedTimeLabel.frame = CGRectMake(
      CGRectGetMinX(self.elapsedTimeLabel.frame),
      CGRectGetMinY(self.elapsedTimeLabel.frame),
      labelSize.width + kLabelWidthPadding, CGRectGetHeight(self.bounds));

  if (CGRectGetWidth(self.elapsedTimeLabel.bounds) >
      CGRectGetWidth(self.progressView.bounds)) {
...
- (void)configureElapsedLabelOriginForElapsedSegment {
  ...
  self.elapsedTimeLabel.frame =
      CGRectMake(CGRectGetMaxX(self.progressView.frame) -
...
- (void)configureElapsedLabelOriginForPendingSegment {
  ...
  self.elapsedTimeLabel.frame =
      CGRectMake(CGRectGetWidth(self.progressView.frame),
                 CGRectGetMinY(self.elapsedTimeLabel.frame),
                 CGRectGetWidth(self.elapsedTimeLabel.frame),
                 CGRectGetHeight(self.elapsedTimeLabel.frame));
}
